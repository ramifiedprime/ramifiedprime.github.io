...
\usepackage{macros}
\upsymbols{Tor,Isom,Div,Hom,Br,inv,WC,nr,red,GL,SL,Aut,tors,Hom,tame,wild}
\upsymbols{Mat,Gal,res,im,coker,Frob,rk,kerl,Res,Ext,disc,Id,FS,cyc,stab,char,tr,End,GL,Jac,Map,small,medium}
\bbsymbols{A,G,Z,Q,R,P,F,C,E}
\scrsymbols{F,C,S,L,M}
\calsymbols{O,T,F,G,C}
\fraksymbols{p,P,m,h,r,q,}
...
\begin{theorem}[Davenport--Heilbronn, 1970s]
...
given by $(e_1,\dots,e_r)\mapsto \prod_{i=1}^r\chi_{p^*}^{e_i}$.  Again this is surjective with kernel $(1,\dots,1)$ as before.  We're then going to define a lift of $\Art_{K,1}$ as follows.
...
\begin{example}
    ...
    \end{pmatrix}=\begin{pmatrix}
        ...
        0&1&1
    \end{pmatrix}.\]
    This has rank $2$ and so we get $\dim \Cl_K[4]=3-1-2=0$.
\end{example}
...
We break this proof into cases.  Firstly we will deal with the number of integers which are not in any grid at all, then we deal with those who are not in a \emph{good} grid.
    ...
    \begin{enumerate}
        ...
        As $\omega(n)\ll \frac{\log H}{\log \log H}$ the last condition implies that $n\geq He^{-c/\log\log H}$ for some $c>0$. So there are at lost $H/\log \log H$ such integers, which is within our proposed bound.
...
\begin{align*}
    ...
    =O(\frac{X}{\log X})+X\sum_{p\leq X}\frac{1}{p}
    =X\log\log X+O(X)
\end{align*}
Then we compute
\begin{align*}
...
\begin{example}
    ...
    \[\sum_{n\leq X}\abs{\sum_{\substack{p\leq X\\\textnormal{prime}}}\art{m}{n}}\ll_\epsilon X^{3/2+\epsilon},\]
    ...
    \[\abs{\sum_{\substack{p\leq X\\\textnormal{prime}}}\art{m}{n}}\ll X^{\frac{1+\epsilon}{2}}\]
...
\begin{rem}
    ...
    \[\abs{\sum_{a\leq A}x_ay_a}\leq \braces{\sum_{x\leq A}x_a^2}^{1/2}\braces{\sum_{y\leq A}y_a^2}^{1/2}.\]
    We will also make use of H\"olders inequality: for fixed $p,q\in\RR_{>1}$ such that $\tfrac{1}{p}+\tfrac{1}{q}=1$
    \[{\sum_{a\leq A}\abs{x_ay_a}}\leq \braces{\sum_{x\leq A}\abs{x_a}^p}^{1/p}\braces{\sum_{y\leq A}\abs{y_a}^q}^{1/q}\]

    Try not to forget these!
\end{rem}
\begin{proof}
    Applying Cauchy--Schwarz with $x_a=1$ and $y_a=\abs{\sum_{\substack{n\leq N\\\textnormal{odd}}}\beta_n\art{m}{n}}$, we have
    \begin{align*}
...
\begin{proof}[Proof of \Cref{thm:FIsieve}]
    ...
    \begin{equation}\label{eq:doubleFI}\abs{\sum_{\substack{m\leq M\\\textnormal{odd}}}\sum_{n\leq N}\alpha_m\beta_n\art{m}{n}}\ll_\epsilon \frac{(MN)^{1+\epsilon}}{\min\set{M^{1/6}, N^{1/6}}}.\end{equation}
    ...
    \[\abs{\sum_{\substack{m\leq M\\\textnormal{odd}}}\sum_{n\leq N}\alpha_m\beta_n\art{m}{n}}\leq \sum_{\substack{m\leq M\\\textnormal{odd}}}
    ...
    Take 
    \[\alpha_m=\begin{cases}
        \frac{\abs{\sum_{n\leq N}\beta_n\art{m}{n}}}{\sum_{n\leq N}\beta_n\art{m}{n}}&\textnormal{if defined}\\
        0&\textnormal{else}
    \end{cases}\]
...
\begin{theorem}[Fouvry--Kl\"uners 2007]
    We have
    \[
        \lim_{X\to\infty} \frac{\#\set{K/\QQ\textnormal{ imag. quad. }\abs{\Delta_K}\leq H,\ 2\Cl_K[4]\cong \FF_2^n}}{\#\set{K/\QQ\textnormal{ imag. quad. }\abs{\Delta_K}\leq H}}=\lim_{r\to\infty}P_{\Mat}(r,n).
    \]
\end{theorem}
...
\begin{theorem}
    ...
    \[\frac{\#\set{x\in X~:~2\Cl_{\QQ(\sqrt{-x})}\cong \FF_2^n}}{\# X}=\lim_{r\to\infty}P(r,n) +o(1),\]
    as $H\to\infty$.
\end{theorem}

We take the moment approach here, it is enough to prove that for each $k\geq 1$
\[\sum_{x\in X}\#2\Cl_{\QQ(\sqrt{-x})}[4]^k\sim c_k\cdot \abs{X}\]
...
\end{pmatrix}
\end{equation}
where the $a_{rr}$ are such that the column sums are zero.  For each vector $\bv\in \FF_2^r$, we have that $\bv$ is in the left kernel if and only if for every $1\leq i\leq r$
\begin{itemize}
    \item if $v_i=0$ then $\prod_{j\textnormal{s.t.}v_j=1}\art{p_i}{p_j}=1$ (by multiplying $\bv$ with the $i$th column of $R_x$)
    \item if $v_i=1$ then $\prod_{j\textnormal{s.t.}v_j=0}\art{p_i}{p_j}=1$ (by multiplying $\bv$ with the $i$th column of $R_x$)
\end{itemize}
\begin{align*}
    \sum_{x\in X'}\#2\Cl_{\QQ(\sqrt{-x})}[4]
    &=\frac{1}{2}\sum_{x\in X'}\sum_{\bv\in \FF_2^r}1_{\bv\in \kerl(R_x)}
    \\&=\frac{1}{2}\sum_{x\in X'}\sum_{\bv\in \FF_2^r}\prod_{i~:~v_i=0}\frac{1}{2}\braces{1+\prod\limits_{\substack{j\\v_j=1}}\art{p_j}{p_i}}\prod_{i~:~v_i=1}\frac{1}{2}\braces{1+\prod\limits_{j~:~v_j=0}\art{p_j}{p_i}}
    \\&=\frac{1}{2^{r+1}}\sum_{\bv\in \FF_2^r}\sum_{x\in X'}\braces{\sum_{\bw\in \FF_2^{\set{i:v_i=0}}}\prod_{i:w_i=1}\prod_{i:v_j=1}\art{p_j}{p_i}}\braces{\sum_{\bw'\in \FF_2^{\set{i:v_i=1}}}\prod_{i:w_i=1}\prod_{i:v_j=0}\art{p_j}{p_i}}
    \\&=\frac{1}{2^{r+1}}\sum_{\bv\in\FF_2^r}
    \sum_{\bw\in \FF_2^{\set{i:v_i=0}}}
    \sum_{\bw'\in\FF_2^{\set{i:v_i=1}}}
    \prod_{i:w_i=1}
    \prod_{i:v_j=1}\art{p_j}{p_i}
    \prod_{i:w_i'=1}
    \prod_{i:v_j=0}\art{p_j}{p_i}
\end{align*}
where here $\kerl(R_x)$ is the left kernel.  Now consider the natural bijection between $\FF_2^r$ and the powerset of $\set{1,\dots,r}$.  Then we exchange $(\bv,\bw,\bw')$ with disjoint sets $S_1,S_2,S_3,S_4$ whose total union is the set $\set{1,\dots,r}$.  Then we get
\[\frac{1}{2^{r+1}}\sum_{S_1\sqcup S_2\sqcup S_3\sqcup S_4=[r]}\sum_{x\in X'}\braces{\frac{x(S_3)x(S_4)}{x(S_1)}}\braces{\frac{x(S_1)x(S_2)}{x(S_4)}}.\]
where $x(S)=\prod_{i\in S}p_i$ for $x=p_1\dots p_r.$  
\begin{definition}
    We say that $1$ is linked with $3$ and $2$ is linked with $4$.  All other pairs are declared to be unlinked.  We also write $i_{\medium}$ for the last index $i$ such that $X_i\subseteq [1,e^{\sqrt{\log(X)}}]$
\end{definition}

\textbf{Case 1:} if there are linked $i,j$ such that $S_i\not\subseteq [i_{\small}]$ and $S_j\not\subseteq [i_{\small}]$ then we can apply the Large sieve over $X_i$ and $X_j$ (exercise: what are the coefficients?) to show that these contributions are small.
...
\textbf{Case 3:} In the remaining cases, we have $(S_1,S_3)\subseteq ([i_\small], [i_\medium])\cup ([i_\medium], [i_\small])$ and the same for $(S_2,S_4)$.  Or we get $\emptyset\in \set{S_1,S_3}$, or $\emptyset\in \set{S_2,S_4}$.  Applying Siegel--Walfisz in all cases we have nontrivial characters, we are left with a main term which correpsonds to $(S_1,S_2)=(\emptyset,\emptyset)$, $(S_1,S_4)=(\emptyset,\emptyset)$, $(S_3,S_4)=(\emptyset,\emptyset)$, $(S_3,S_2)=(\emptyset,\emptyset)$.

\ross{Below is an adaptation of the explanation above from immediately after equation \eqref{eq:RedeiRevamp}.  At worst, we hope the reader finds this addition to not be unhelpful and if they are comfortable with the above then they should ignore it.


For each vector $\bv\in \FF_2^r$, we want to understand when $\bv$ is in the left kernel of $R_x$.  Note that being in the left Kernel is equivalent to $\bv\cdot R_x(i)=0$ for all $i$, where $R_x(i)$ denotes the $i$th column of the R\'edei matrix.  For each $i$, now that we have written down $a_{ii}$, we can see that
\[\bv\cdot R_x(i)=\art{\prod_{j\neq i}p_j^{v_j+v_i}}{p_i}.\]
Hence the inficator function $1_{\bv\in \kerl(R_x)}$ is given explicitly by
\begin{align*}
...
Let us now expand these products.  For for now $\bv\in \FF_2^r$ and $x=p_1\dots p_r\in X'$.  We let $V_0=V_0(\bv)$ denote the set of indices $i\in\set{1,\dots,r}$ such that $v_i=0$, and similarly $V_1$, and moreover for any subset $S\subseteq \set{1,\dots,r}$ and element $x=p_1\dots p_r\in X'$ we write $x(S)=\prod_{i\in S}p_i$.  Then we expand the products in the indicator function to obtain
\begin{align*}
...
\[\sum_{\bv\in\FF_2^r}1_{\bv\in \kerl(R_x)}=\frac{1}{2^r}\sum_{S_0\sqcup S_1\sqcup S_2\sqcup S_3=[r]}\art{x(S_1)x(S_3)}{x(S_0)}\art{x(S_0)x(S_2s)}{x(S_1)}.\]
...
\textbf{Case 3:} In the remaining cases, we have $(S_0,S_3)\subseteq ([i_\small], [i_\medium])\cup ([i_\medium], [i_\small])$ and the same for $(S_1,S_2)$.  Or we get $\emptyset\in \set{S_0,S_3}$, or $\emptyset\in \set{S_1,S_2}$.  Applying Siegel--Walfisz in all cases we have nontrivial characters, we are left with a main term which correpsonds to a sum of the four cases where our characters are forced to be constant: $(S_0,S_2)=(\emptyset,\emptyset)$, $(S_1,S_3)=(\emptyset,\emptyset)$, $(S_0,S_1)=(\emptyset,\emptyset)$, $(S_2,S_3)=(\emptyset,\emptyset)$.  Note that in the first three, the character is trivial, and in the last it is controlled by congruence modulo $4$ (which is determined here!).
}


\lecture{}{}
\section{Repeated Cauchy--Schwarz}
Today we'll see a key new ingredient in this area:  repeated Cauchy--Schwarz.  Our setup is as follows.  Say we have three sets of primes
\begin{itemize}
    \item $X_1=\set{H<p<2H~:~p\equiv 3\mod 4}$
    \item $X_2=\set{2H<q<3H~:~q\equiv 1\mod 4}$
    \item $X_3=\set{3H<r<4H~:~r\equiv 1\mod 4}$ 
\end{itemize}
and as usual the product $X=X_1\times X_2\times X_3$.

Recall for each $x=(p,q,r)\in X$ with associated quadratic field $K$ we already have the R\'edei matrix determining what comes from $4$-torsion
\[\begin{tikzcd}
...
Set
\[Y:=\set{x=(p,q,r)~:~R_x=0},\]
i.e. this is the set of triples such that $\art{p}{q}=\art{p}{r}=\art{q}{r}=+1$.  This is \emph{not} a subgrid.  Understanding the left kernel of $R_x$ told us precisely what $2\Cl_K[4]$ was.  We now want to understand what happens at this next layer and how we should compute $\widetilde{\Art_{x,2}}$.  At this layer, we have a diagram

\[\begin{tikzcd}
    \varphi_x^{-1}(2\Cl_K[4])\times \psi_x^{-1}(2\Cl^\vee_K[4])\ar[r,"\widetilde{\Art_{x,2}}"]\ar[d,"\varphi_x\times \psi_x"]&\set{\pm 1}\ar[d,equals]\\
    2\Cl_K[4]\times 2\Cl_K^\vee[4]\ar[r,"\Art_{x,1}"]&\set{\pm 1}.
\end{tikzcd}\]
Here, by definition, $\widetilde{\Art_{x,2}}(\bv,\bw)=\psi_x^4(\bw)(\Frob_{\varphi_x(\bv)})$, where $\psi_x^4(\bw)\in Z^1_{\nr}(G_\QQ, N(-x)[4])$ satisfies $2\psi_x^4(\bw)=\psi_x(\bw)$.

Our goal today is to discuss the following claim:
\begin{proposition}
\[\abs{\sum_{x\in Y}\widetilde{\Art_{x,2}(\bv,\bw)}}=o(H^3/\log(H)^3).\] 
\end{proposition}
Actually it is possible to reach $o(H^{3-\delta})$, but we'll not work that hard.

\noindent \textbf{Important: Today we will specialise to the setting $\bv=(1,0,0)$ and $\bw=(0,1,0)$!!}
...
We will begin by looking for relations which constrain $\psi_x^4$ as an element of $\Map(G_\QQ, N(-x))$, where we recall that $N(-x)$ is the abelian group $\QQ_2/\ZZ_2$ upon which we act by Galois via the quadratic character $\chi_x$.

We note the following repeated application of Cauchy--Schwarz from excercise sheet 3.

\begin{lemma}\label{lem:repeatedCS}
    \[\sum_{x\in Y}\widetilde{\Art_{x,2}}(\bv,\bw)\leq \abs{X}^{3/4}\abs{\sum_{\substack{p_1,p_2\in X_1\\q_1,q_2\in X_2\\r_1,r_2\in X_3}}\prod_{i=1}^2\prod_{j=1}^2\prod_{k=1}^2\widetilde{\Art_{x,2}}(\bv,\bw)}^{1/8}\]
\end{lemma}
...
\begin{proof}
    ...
    \begin{align*}
        ...
        +\chi_{-p_2q_1r}(\sigma)\chi_{q_1}(\tau)+\chi_{-p_2q_2r}\chi_{q_2}(\tau)\\
        &=\chi_{p_1p_2}(\sigma)\chi_{q_1}(\tau)\chi_{p_1p_2}(\sigma)\chi_{q_2}(\tau)\\
        &=\chi_{p_1p_2}(\sigma)\chi_{q_1q_1}(\tau).
    \end{align*}
...
Hence, there is a continuous $1$-cochain $\phi_{p_1p_2q_1q_2}:G_\QQ\to \FF_2$ such that 
\begin{equation}\label{eq:secondcoboundary}
d\phi_{p_1p_2,q_1q_2}(\sigma,\tau)=\chi_{p_1p_2}(\sigma)\chi_{q_1q_2}(\tau)
\end{equation}
\begin{definition}
    We define $\theta\in H^2(\FF_2^2,\FF_2)$ by $(\sigma,\tau)\mapsto \pi_1(\sigma)\pi_2(\sigma)$ where $\pi_i:\FF_2^2\to \FF_2$ are the natural coordinate projections.  We define 
...
Define $E=\QQ(\sqrt{p_1p_2},\sqrt{q_1q_2})$.  Note that $\ker(d)=\Hom(G_\QQ,\FF_2)$, and so the set of possible $\phi_{p_1p_2,q_1q_2}$ differ by a quadratic character:  for all $\chi\in \Hom(G_\QQ,\mu_2)$
\[d(\phi_{p_1p_2,q_1q_2}+\chi)(\sigma,\tau)=\chi_{p_1p_2}(\sigma)+\chi_{q_1q_2}(\tau)\]
In fact, we may choose $\phi_{p_1p_2,q_1q_2}$ to be unramified over $E$  Indeed, let $\sigma_p$ denote a generator of tame inertia at an odd prime $p$, and write $e_p=\operatorname{ord}(\psi(\sigma_p))$ (which turns out to be the ramification degree of $p$ in $\bar{\QQ}^{\ker(\psi)}/E$).  Then away from $p_1p_2q_1q_2$ we can twist to ensure that $\psi(\sigma_p)=(*,0,0)$.  If $*=0$ then we get $\psi(\sigma_p)=0$ so $p$ is unramified, else if $*=1$ then we add $\chi_{p^*}$.

We now write $\phi_{p_1p_2,q_1q_2}^\nr$ for our choice of cochain which is unramified over $E$.
\begin{theorem}
    Let $p_1,p_2\in X_1$, $q_1,q_2\in X_2$, $r\in X_3$.  Assume that $p_iq_jr\in Y$.  Then setting $C:=\set{p_1,p_2}\times \set{q_1,q_2}\times \set{r}$, and making any choice of $\phi_{p_1p_2q_1q_2}^\nr$ we obtain that

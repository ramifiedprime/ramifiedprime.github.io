...
\upsymbols{Mat,Gal,res,im,coker,Frob,rk,Res,Ext,disc,Id,FS,cyc,stab,char,tr,End,GL,Jac,Map,small,medium}
...
\[\begin{tikzcd}
    ...
    2\Cl_K[4]\times 2\Cl_K^\vee[4]\ar[r,"\Art_{x,2}"]&\set{\pm 1}.
\end{tikzcd}\]
Let us work out a construction of the pairing.  By definition, $\widetilde{\Art_{x,2}}(\bv,\bw)$ is constructed by choosing an element $f_{x,\bw}\in \Cl_K^\vee[4]$ such that $2f_{x,\bw}=\psi_x(\bw)$ and then from this we have $\widetilde{\Art_{x,2}}(\bv,\bw)=f_{x,\bw}(\psi_x(\bv))$.  Via \Cref{thm:L1thm1} we can lift this choice of $f_{x,\bw}$ back to an element of $Z^1(\Gal(H_K/\QQ),N(-x)[4])$ which restricts to $f_{x,\bw}$ and then further via \Cref{def:Z1nr} we inflate to an element $\psi_{x}^4(\bw)\in Z^1_{\nr}(G_\QQ, N(-x)[4])$.  In particular, $\psi_{x}^4(\bw)$ is well defined on $\Gal(H_K/\QQ)$ and when restricted to $\Gal(H_K/K)\cong \Cl_K$ we have $2\psi_{x}^4(\bw)=\psi_{x}(\bw)$.\ross{One day I will come back and change $\psi_{x}^4(\bw)$ to $\psi_{x,\bw}^4$ which seems slightly tidier, but that day has not yet come.}
...
\begin{theorem}\label{thm:cubesum}
    ...
    where $\Frob_\fr$ is the genuine from $\QQ$ in an unramified extension.
\end{theorem}
...
\begin{proof}[Proof of \Cref{thm:cubesum}]
    ...
    \[\sum_{x\in C}\psi_x^4(\bw)-\phi_{p_1p_2,q_1q_2}^\nr\in \ker(d)=\Hom(G_\QQ,\FF_2).\]
    What can we say about this character $\chi$?  Well, since we've made our $\phi_{p_1p_2,q_1q_2}^\nr$ unramified\ross{why is the sum unramified away from here?}, this character $\chi$ can only ramify at $p_1,p_2,q_1,q_2,r$.  But $\fr'$ splits for all such quadratic characters since $\bv=(0,0,1)$ is in the left kernel of $R_x$ for all $x\in C$, and finally
